...
class NodeSimulator:
    ...
    def update_metrics(self, operation_size: int, is_source: bool):
        ...
        # Simulate disk usage
        self.disk_usage = min(100, self.disk_usage + (operation_size / (self.config.disk_gb * 1024 * 1024 * 1024)) * 100)
...
class NetworkSimulator:
    ...
    async def simulate_transfer(self, source: str, dest: str, size: int) -> Tuple[bool, float]:
        """
...
class ScenarioSimulator:
    ...
    def _generate_operation(self) -> Tuple[str, str, str, int]:
        ...
        source_nodes = [node_id for node_id in self.nodes if node_id.startswith(source_region)]
        dest_nodes = [node_id for node_id in self.nodes if node_id.startswith(dest_region)]
        ...
        if self.config.workload_pattern == 'burst':
            ...
            if random.random() < 0.8:
                size = random.randint(
                    self.config.data_size_range[0],
                    self.config.data_size_range[0] * 10
                )
            else:
                size = random.randint(
                    self.config.data_size_range[1] // 2,
    ...
    async def _simulate_operation(self) -> OperationResult:
        ...
        if op_type == 'write':
            ...
            available_nodes = [n for n in self.nodes if n != dest and not self.nodes[n].is_failed]
