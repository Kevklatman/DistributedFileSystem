...
metrics = SystemMetricsCollector(instance_id=socket.gethostname())
    ...
    try:
        if not request.path.startswith(
            ...
            app.request_queue.add_request(
                {"path": request.path, "method": request.method, "time": start_time}
            )
    ...
    try:
        if not request.path.startswith(("/metrics", "/static")):
            ...
            metrics.record_request(
                ...
                duration=duration,
                error=(
                    str(response.status_code) if response.status_code >= 400 else None
                ),
            )
        ...
        return jsonify(
            {
                ...
                "metrics": {
                    ...
                    "queue": {"length": app.request_queue.get_length()},
                },
            }
        )
    except Exception as e:
