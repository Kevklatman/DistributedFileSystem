...
REQUEST_TOTAL = Counter(
    "dfs_request_total", "Total number of requests", ["endpoint", "status", "instance"]
)
...
REQUEST_QUEUE_LENGTH = Gauge(
    "dfs_request_queue_length", "Number of requests in queue", ["instance"]
)

# File Operation Metrics
FILE_OPERATIONS = Counter(
    "dfs_file_operations_total", "Number of file operations", ["operation", "instance"]
)
...
# Storage Metrics
STORAGE_USAGE = Gauge(
    "dfs_storage_usage_bytes", "Current storage usage in bytes", ["instance", "node_id"]
)
...
# Cache Metrics
CACHE_HITS = Counter("dfs_cache_hits_total", "Number of cache hits", ["instance"])

CACHE_MISSES = Counter("dfs_cache_misses_total", "Number of cache misses", ["instance"])
...
CPU_USAGE = Gauge("dfs_cpu_usage_percent", "CPU usage percentage", ["instance"])

MEMORY_USAGE = Gauge("dfs_memory_usage_bytes", "Memory usage in bytes", ["instance"])
...
class MetricsCollector:
    ...
    def record_request(self, endpoint, status, latency):
        ...
        REQUEST_TOTAL.labels(
            endpoint=endpoint, status=status, instance=self.instance_id
        ).inc()

        REQUEST_LATENCY.labels(endpoint=endpoint, instance=self.instance_id).observe(
            latency
        )
    ...
    def record_file_operation(self, operation, duration):
        ...
        FILE_OPERATIONS.labels(operation=operation, instance=self.instance_id).inc()

        FILE_OP_DURATION.labels(operation=operation, instance=self.instance_id).observe(
            duration
        )
    ...
    def update_storage_metrics(self, usage_bytes, capacity_bytes, node_id):
        ...
        STORAGE_USAGE.labels(instance=self.instance_id, node_id=node_id).set(
            usage_bytes
        )

        STORAGE_CAPACITY.labels(instance=self.instance_id, node_id=node_id).set(
            capacity_bytes
        )

    def record_network_io(self, direction, bytes_count):
        """Record network I/O"""
        NETWORK_IO.labels(direction=direction, instance=self.instance_id).inc(
            bytes_count
        )
    ...
    def update_replication_metrics(self, target_node, lag_seconds, queue_length):
        ...
        REPLICATION_LAG.labels(instance=self.instance_id, target_node=target_node).set(
            lag_seconds
        )
