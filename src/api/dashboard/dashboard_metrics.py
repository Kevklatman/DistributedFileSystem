...
def get_node_metrics(node_id: str, timeframe: str = "5m") -> NodeMetrics:
    ...
    try:
        # System metrics
        cpu = query_prometheus(f'avg_over_time(dfs_system_cpu_percent{{instance="{node_id}"}}[{timeframe}])')
        memory = query_prometheus(f'avg_over_time(dfs_system_memory_percent{{instance="{node_id}"}}[{timeframe}])')
        ...
        storage_used = query_prometheus(f'dfs_storage_used_bytes{{instance="{node_id}"}}')
        
        # Cache metrics
        cache_hits = query_prometheus(f'increase(dfs_cache_hits_total{{instance="{node_id}"}}[{timeframe}])')
        cache_misses = query_prometheus(f'increase(dfs_cache_misses_total{{instance="{node_id}"}}[{timeframe}])')
        
        # Request metrics
        total_requests = query_prometheus(f'increase(dfs_requests_total{{instance="{node_id}"}}[{timeframe}])')
        failed_requests = query_prometheus(f'increase(dfs_requests_failed_total{{instance="{node_id}"}}[{timeframe}])')
        request_duration = query_prometheus(f'rate(dfs_request_duration_seconds_sum{{instance="{node_id}"}}[{timeframe}])')
        request_p95 = query_prometheus(f'histogram_quantile(0.95, rate(dfs_request_duration_seconds_bucket{{instance="{node_id}"}}[{timeframe}]))')
        
        # File operations
        reads = query_prometheus(f'increase(dfs_file_reads_total{{instance="{node_id}"}}[{timeframe}])')
        writes = query_prometheus(f'increase(dfs_file_writes_total{{instance="{node_id}"}}[{timeframe}])')
        deletes = query_prometheus(f'increase(dfs_file_deletes_total{{instance="{node_id}"}}[{timeframe}])')
        op_duration = query_prometheus(f'rate(dfs_file_operation_duration_seconds_sum{{instance="{node_id}"}}[{timeframe}])')
        
        # Replication metrics
        rep_lag = query_prometheus(f'dfs_replication_lag_seconds{{instance="{node_id}"}}')
        rep_queue = query_prometheus(f'dfs_replication_queue_length{{instance="{node_id}"}}')
        
        # Network metrics
        network_in = query_prometheus(f'rate(dfs_network_received_bytes_total{{instance="{node_id}"}}[{timeframe}])')
        network_out = query_prometheus(f'rate(dfs_network_sent_bytes_total{{instance="{node_id}"}}[{timeframe}])')
        ...
        return NodeMetrics(
            ...
            storage=StorageMetrics(
                ...
                available_bytes=extract_value(storage_total) - extract_value(storage_used),
                usage_percent=(extract_value(storage_used) / extract_value(storage_total) * 100) if extract_value(storage_total) > 0 else 0
            ),
            cache=CacheMetrics(
                hits=int(extract_value(cache_hits)),
                misses=int(extract_value(cache_misses)),
                hit_rate=extract_value(cache_hits) / (extract_value(cache_hits) + extract_value(cache_misses)) if (extract_value(cache_hits) + extract_value(cache_misses)) > 0 else 1.0
            ),
            requests=RequestMetrics(
...
async def get_metrics(timeframe: str = "5m"):
    ...
    try:
        ...
        nodes_result = query_prometheus('count(dfs_system_uptime_seconds) by (instance)')
...
async def get_health():
    ...
    try:
        ...
        # Check storage usage
        storage_result = query_prometheus('max(dfs_storage_used_bytes / dfs_storage_capacity_bytes * 100)')
        ...
        # Check error rate
        error_result = query_prometheus('max(rate(dfs_requests_failed_total[5m]) / rate(dfs_requests_total[5m]))')
        ...
        if all_healthy and not storage_critical and not replication_critical and not error_critical:
            return {
                "status": "healthy",
