...
# Storage configuration
STORAGE_CONFIG = {
    "local": {"storage_dir": os.getenv("LOCAL_STORAGE_DIR", "./storage")},
    "aws": AWS_CONFIG,
}
...
class ConfigurationService:
    ...
    def __init__(self):
        self.config = SystemConfig(
            ...
            max_volume_size=int(
                os.getenv("MAX_VOLUME_SIZE", str(1024 * 1024 * 1024 * 1024))
            ),  # 1TB
    ...
    def _update_system_status(self):
        ...
        healthy_nodes = sum(
            1
            for status in self._health.node_statuses.values()
        ...
        if healthy_nodes == total_nodes:
            ...
            self._health.message = (
                f"Insufficient healthy nodes ({healthy_nodes}/{total_nodes})"
            )
