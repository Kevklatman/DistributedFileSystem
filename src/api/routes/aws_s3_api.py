...
class AWSS3ApiHandler(BaseS3Handler):
    ...
    def register_aws_routes(self, blueprint):
            ...
            try:
                ...
                if isinstance(result, bool):
                    return format_error_response(
                        "ListBucketError", "Failed to list buckets", 404
                    )
                return result
        ...
        def create_bucket(bucket_name):
            ...
            try:
                ...
                if not result:
                    return format_error_response(
                        "BucketCreationError", "Failed to create bucket", 400
                    )
        ...
        def delete_bucket(bucket_name):
            ...
            try:
                ...
                if not result:
                    return format_error_response(
                        "BucketDeletionError", "Failed to delete bucket", 404
                    )
        ...
        def list_objects(bucket_name):
            ...
            try:
                ...
                if isinstance(result, bool):
                    return format_error_response(
                        "ListObjectsError", "Failed to list objects", 404
                    )
                return result
            except Exception as e:
        ...
        def put_object(bucket_name, key):
            ...
            try:
                ...
                if not result:
                    return format_error_response(
                        "PutObjectError", "Failed to upload object", 400
                    )
        ...
        def get_object(bucket_name, key):
            ...
            try:
                ...
                if not result:
                    return format_error_response(
                        "GetObjectError", "Object not found", 404
                    )
        ...
        def delete_object(bucket_name, key):
            ...
            try:
                ...
                if not result:
                    return format_error_response(
                        "DeleteObjectError", "Object not found", 404
                    )
                return Response("", status=204)
            except Exception as e:
    ...
    def create_bucket(self, bucket_name: str, location: str) -> bool:
        ...
        return self.handle_storage_operation(
            "create_bucket", bucket_name=bucket_name, location=location
        )
    ...
    def list_objects(self, bucket_name: str, **params) -> Union[Dict[str, Any], bool]:
        """List objects in a bucket."""
        return self.handle_storage_operation(
            "list_objects", bucket_name=bucket_name, **params
        )
    ...
    def get_object(
        ...
        """Download an object."""
        return self.handle_storage_operation(
            "get_object", bucket_name=bucket_name, object_key=object_key
        )
