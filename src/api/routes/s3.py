...
class S3ApiHandler:
    ...
    def create_error_response(
        self, code: str, message: str, status_code: int = 500
    ) -> Response:
    ...
    def register_basic_routes(self, blueprint: Blueprint):
            ...
            try:
                ...
                response = {
                    "ListAllMyBucketsResult": {
                        "Owner": {"ID": "dfs-owner", "DisplayName": "DFS Owner"},
                        "Buckets": {
                            "Bucket": [
                                {
                                    "Name": bucket.name,
                ...
                return Response(
                    xmltodict.unparse(response), content_type="application/xml"
                )
        ...
        def put_object(bucket, key):
            ...
            try:
                ...
                logger.error(
                    f"Error uploading object {key} to bucket {bucket}: {str(e)}"
                )
                return self.create_error_response("InternalError", str(e))
        ...
        def get_object(bucket, key):
            ...
            try:
                ...
                return Response(
                    ...
                    content_type=obj.metadata.get(
                        "Content-Type", "application/octet-stream"
                    ),
                )
            except Exception as e:
                logger.error(
                    f"Error downloading object {key} from bucket {bucket}: {str(e)}"
                )
                return self.create_error_response("InternalError", str(e))
        ...
        def delete_object(bucket, key):
            ...
            try:
                ...
                logger.error(
                    f"Error deleting object {key} from bucket {bucket}: {str(e)}"
                )
                return self.create_error_response("InternalError", str(e))
        ...
        def list_objects(bucket):
            ...
            try:
                ...
                objects = self.fs_manager.list_objects(
