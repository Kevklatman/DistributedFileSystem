...
class HybridStorageManager:
    ...
    async def write_file(
        self, file_path: str, data: bytes, protocol: Optional[str] = None
    ) -> None:
    ...
    async def _write_s3(self, file_path: str, data: bytes) -> None:
        ...
        await asyncio.get_event_loop().run_in_executor(
            ...
            lambda: self.s3_client.put_object(
                Bucket="test-bucket", Key=file_path, Body=data
            ),
        )
    ...
    async def _read_s3(self, file_path: str) -> bytes:
        ...
        response = await asyncio.get_event_loop().run_in_executor(
            None, lambda: self.s3_client.get_object(Bucket="test-bucket", Key=file_path)
        )
            ...
            if protocol == "local":
                ...
                await asyncio.get_event_loop().run_in_executor(
                    ...
                    lambda: self.s3_client.delete_object(
                        Bucket="test-bucket", Key=file_path
                    ),
                )
            else:
    ...
    async def list_files(self, protocol: Optional[str] = None) -> list:
        ...
        try:
            if protocol == "local":
                ...
                response = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: self.s3_client.list_objects_v2(Bucket="test-bucket")
                )
    ...
    async def switch_protocol(
        self, file_path: str, from_protocol: str, to_protocol: str
    ) -> None:
    ...
    async def failover(
        self, file_path: str, primary_protocol: str, backup_protocol: str
    ) -> bytes:
        ...
        try:
            ...
            logger.warning(
                f"Primary protocol {primary_protocol} failed, failing over to {backup_protocol}"
            )
            try:
                ...
                raise Exception(
                    f"Both primary and backup protocols failed: {primary_error}, {backup_error}"
                )
