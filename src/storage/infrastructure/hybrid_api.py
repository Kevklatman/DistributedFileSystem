...
location_model = api.model(
    ...
    {
        "type": fields.String(
            required=True, enum=["on_prem", "aws_s3", "azure_blob", "gcp_storage"]
        ),
    ...
    def post(self):
        ...
        try:
            pool = storage_manager.create_storage_pool(
                name=data["name"], location=location, capacity_gb=data["capacity_gb"]
            )
            return {
                "id": pool.id,
                "name": pool.name,
