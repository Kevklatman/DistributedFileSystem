...
class LoadManager:
    ...
    def can_handle_request(self) -> bool:
        ...
        return (
            metrics.cpu_usage < self.max_cpu_threshold
            and metrics.memory_usage < self.max_memory_threshold
            and current_rps < self.max_requests_per_second
        )
    ...
    def get_current_load(self) -> float:
        ...
        weights = {"cpu": 0.4, "memory": 0.3, "disk_io": 0.15, "network_io": 0.15}
    ...
    def get_current_metrics(self) -> LoadMetrics:
        ...
        disk_io_bytes = (current_disk_io.read_bytes + current_disk_io.write_bytes) - (
        ...
        network_io_bytes = (current_net_io.bytes_sent + current_net_io.bytes_recv) - (
        ...
        old_timestamps = [
            ts for ts in self.metrics_history.keys() if current_time - ts > 300
        ]
    ...
    def predict_load_trend(self, window_seconds: float = 60.0) -> Optional[float]:
        ...
        time_points = [
            m.timestamp - recent_metrics[0].timestamp for m in recent_metrics
        ]
