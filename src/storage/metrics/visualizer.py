...
class MetricsVisualizer:
    ...
    def plot_system_metrics(self, save_path: str = None) -> None:
        ...
        ax3.bar(['Disk I/O Rate'], [system['disk_io_rate'] / 1024 / 1024], color='orange')
        ax3.set_ylabel('MB/s')
        ax3.set_title('Disk I/O Rate')

        ax4.bar(['Network I/O Rate'], [system['network_io_rate'] / 1024 / 1024], color='red')
        ax4.set_ylabel('MB/s')
    ...
    def plot_operation_latencies(self, save_path: str = None) -> None:
        ...
        avg_latencies = [op_stats['avg_latency'] * 1000 for op_stats in operations.values()]  # Convert to ms
        ...
        # Min/Max latencies
        min_latencies = [op_stats['min_latency'] * 1000 for op_stats in operations.values()]
        max_latencies = [op_stats['max_latency'] * 1000 for op_stats in operations.values()]
    ...
    def plot_cache_performance(self, save_path: str = None) -> None:
        ...
        # Hit rate
        ax1.pie([cache['hit_rate'], 1 - cache['hit_rate']],
                labels=['Hits', 'Misses'],
                autopct='%1.1f%%',
                colors=['green', 'red'])
    ...
    def generate_report(self, report_dir: str) -> None:
        ...
        metrics = self.collector.get_metrics()
        with open(os.path.join(report_dir, f'metrics_summary_{timestamp}.txt'), 'w') as f:
