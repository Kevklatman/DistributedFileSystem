...
class StorageMetricsCollector:
    ...
    def _update_storage_metrics(self):
        ...
        try:
            ...
            STORAGE_USAGE.labels(
                instance=self.instance_id, node_id=self.node_id, path=self.storage_path
            ).set(usage.used)
    ...
    def record_operation(self, operation: str):
        ...
        return FileOperationTracker(
            instance_id=self.instance_id, node_id=self.node_id, operation=operation
        )

    def record_file_size(self, size_bytes: int):
        """Record a file size observation"""
        FILE_SIZES.labels(instance=self.instance_id, node_id=self.node_id).observe(
            size_bytes
        )
    ...
    def record_bandwidth(self, direction: str, bytes_count: int):
        ...
        STORAGE_BANDWIDTH.labels(
            instance=self.instance_id, node_id=self.node_id, direction=direction
        ).inc(bytes_count)
...
class FileOperationTracker:
    ...
    def __enter__(self):
        ...
        FILE_OPERATIONS.labels(
            instance=self.instance_id, node_id=self.node_id, operation=self.operation
        ).inc()
        return self
...
def create_metrics_collector(
    instance_id: str, node_id: str, storage_path: str
