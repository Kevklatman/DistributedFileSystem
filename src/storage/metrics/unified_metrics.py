...
class UnifiedMetricsCollector:
    ...
    def _init_metrics(self):
        ...
        self.system_cpu = Gauge(
            "dfs_system_cpu_percent", "CPU usage percentage", ["instance", "node_id"]
        )
    ...
    def update_system_metrics(self):
        ...
        try:
            ...
            self.system_cpu.labels(instance=self.instance_id, node_id=self.node_id).set(
                cpu_percent
            )
            ...
            self.system_memory.labels(
                instance=self.instance_id, node_id=self.node_id, type="used"
            ).set(memory.percent)
            self.system_memory.labels(
                instance=self.instance_id, node_id=self.node_id, type="available"
            ).set(100 - memory.percent)

            # Load average
            load1, load5, load15 = psutil.getloadavg()
            self.system_load.labels(
                instance=self.instance_id, node_id=self.node_id, interval="1m"
            ).set(load1)
            self.system_load.labels(
                instance=self.instance_id, node_id=self.node_id, interval="5m"
            ).set(load5)
            self.system_load.labels(
                instance=self.instance_id, node_id=self.node_id, interval="15m"
            ).set(load15)
            ...
            self.system_uptime.labels(
                instance=self.instance_id, node_id=self.node_id
            ).set(uptime)
    ...
    def update_storage_metrics(self, mount_point="/"):
        ...
        try:
            ...
            self.storage_capacity.labels(
                instance=self.instance_id, node_id=self.node_id, mount_point=mount_point
            ).set(disk.total)
            ...
            if disk_io:
                self.storage_iops.labels(
                    instance=self.instance_id, node_id=self.node_id, operation="read"
                ).inc(disk_io.read_count)
                self.storage_iops.labels(
                    instance=self.instance_id, node_id=self.node_id, operation="write"
        ...
        try:
            ...
            for interface, counters in net_io.items():
                self.network_received.labels(
                    instance=self.instance_id, node_id=self.node_id, interface=interface
    ...
    def record_request(
        self, endpoint: str, method: str, duration: float, error: str = None
    ):
    ...
    def record_cache_operation(self, hit: bool, cache_type: str = "memory"):
        ...
        if hit:
            self.cache_hits.labels(
                instance=self.instance_id, node_id=self.node_id, cache_type=cache_type
            ).inc()
        else:
    ...
    def record_file_operation(self, operation: str, duration: float, error: str = None):
        ...
        self.file_ops.labels(
            instance=self.instance_id, node_id=self.node_id, operation=operation
        ).inc()
    ...
    def update_replication_metrics(
        self, target_node: str, lag: float, queue_length: int
    ):
        """Update replication metrics"""
        self.replication_lag.labels(
            instance=self.instance_id, node_id=self.node_id, target_node=target_node
        ).set(lag)
    ...
    def record_replication_error(self, error_type: str):
        ...
        self.replication_errors.labels(
            instance=self.instance_id, node_id=self.node_id, error_type=error_type
        ).inc()
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            ...
            self.metrics.record_file_operation(
                self.operation_type, duration, error=error_type
            )
        else:
            self.metrics.record_file_operation(self.operation_type, duration)
