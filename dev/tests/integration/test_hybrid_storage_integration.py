...
class TestHybridStorageIntegration:
    @pytest.mark.asyncio
    async def test_data_write_and_tiering(
        self, hybrid_manager, active_node, cluster_manager
    ):
        ...
        policy = CloudTieringPolicy(
            cold_tier_after_days=1, archive_tier_after_days=7, delete_after_days=30
        )

        # Create volume with policy
        volume = await active_node.create_volume(
            volume_id=volume_id, size_bytes=len(test_data), tiering_policy=policy
        )

        # Write data
        await active_node.write_data(volume_id=volume_id, offset=0, data=test_data)

        # Verify data is written to local storage
        local_data = await active_node.read_data(
            volume_id=volume_id, offset=0, length=len(test_data)
        )
    ...
    @pytest.mark.asyncio
    async def test_data_replication_and_consistency(
        self, hybrid_manager, active_node, cluster_manager
    ):
        ...
        protection = DataProtection(replica_count=2, consistency_level="strong")

        # Create volume with protection
        volume = await active_node.create_volume(
            volume_id=volume_id, size_bytes=len(test_data), protection=protection
        )

        # Write data with strong consistency
        await active_node.write_data(
            volume_id=volume_id, offset=0, data=test_data, consistency="strong"
        )
    ...
    async def test_cache_behavior(self, hybrid_manager, active_node):
        ...
        # Create volume
        volume = await active_node.create_volume(
            volume_id=volume_id, size_bytes=len(test_data)
        )
