...
class TestHybridStoragePerformance:
            ...
            for protocol in protocols:
                ...
                for _ in range(10):
                    ...
                    await hybrid_storage_manager.write_file(
                        file_path, test_data, protocol=protocol
                    )
    ...
    @pytest.mark.asyncio
    async def test_failover_performance(
        self, hybrid_storage_manager, haproxy_container
    ):
    ...
    async def test_generate_performance_report(
        self, tmp_path, hybrid_storage_manager, haproxy_container
    ):
        ...
        report = {
            ...
            "summary": {
                "avg_write_speed": sum(self.performance_metrics.get("write_speeds", []))
                / len(self.performance_metrics.get("write_speeds", [1])),
                "avg_read_speed": sum(self.performance_metrics.get("read_speeds", []))
                / len(self.performance_metrics.get("read_speeds", [1])),
                "protocol_switch_overhead": self.performance_metrics.get(
                    "protocol_switch_time", 0
                ),
