...
def parse_args():
    ...
    create_parser.add_argument(
        "--storage-size", default="100Gi", help="Size of storage volume"
    )
    create_parser.add_argument(
        "--node-selector",
        default="",
    ...
    create_parser.add_argument(
        "--zone", default="default", help="Availability zone for the node"
    )

    # Monitor command
    monitor_parser = subparsers.add_parser(
        "monitor", help="Monitor node health and metrics"
    )
    ...
    deploy_parser.add_argument(
        "--namespace", default="default", help="Kubernetes namespace to deploy to"
    )
    deploy_parser.add_argument(
        "--config", type=str, required=True, help="Path to cluster configuration YAML"
    )
    deploy_parser.add_argument(
        "--registry", default="docker.io", help="Container registry to use"
    )
